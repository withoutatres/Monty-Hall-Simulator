...
with st.sidebar:
    ...
    max_open = n_doors - 2
    n_opened = st.slider("Doors host opens (k)",  1, max_open, min(S.n_opened, max_open), key="sb_nopened")

    if n_doors != S.n_doors or n_opened != S.n_opened:
        S.n_doors  = n_doors
        S.n_opened = n_opened
        S.phase = "pick"; S.player_door = None; S.car_door = None
        S.opened_doors = []; S.final_door = None; S.won = None
...
with tab_game:
    ...
    phase_info = {
        ...
        "decide":  ("02 / STAY OR SWITCH?",    f"The host opened {n_opened} goat door(s). Click any remaining door — your original pick to stay, or another to switch."),
        "outcome": ("03 / OUTCOME",            ""),
    }
    ...
    for row in range(rows):
        ...
        for ci in range(COLS):
            ...
            disabled = (
                is_opened
                or S.phase == "outcome"
                or (S.phase == "decide" and is_opened)
            )
            ...
            with cols_ui[ci]:
                clicked = st.button(
                    ...
                    type="primary" if (is_player and S.phase in ("decide",)) else "secondary",
                )
                st.markdown(
            ...
            if clicked:
                if S.phase == "pick":
                    ...
                    S.history.append(dict(
                        round=S.round_num, n_doors=n_doors, n_opened=n_opened,
                        player_door=S.player_door, car_door=S.car_door,
                        final_door=d, action=action, won=S.won,
                    ))
                    S.phase = "outcome"
                    st.rerun()
    ...
    if S.history:
        ...
        if len(S.history) >= 2:
            ...
            fig.add_trace(go.Scatter(x=xs, y=cum_stay, mode="lines+markers", name="Stay win rate",
            ...
            fig.add_trace(go.Scatter(x=xs, y=cum_sw, mode="lines+markers", name="Switch win rate",
...
with tab_theory:
    ...
    st.markdown("<p style='color:#6b6b80;font-size:0.88rem;margin-top:-0.5rem'>Adjust sliders in the sidebar to explore any N-door, k-open scenario.</p>", unsafe_allow_html=True)
    ...
    with col_l:
        ...
        fig_bar.add_trace(go.Bar(
            ...
            marker_color="rgba(255,255,255,0.1)", marker_line_color="rgba(255,255,255,0.25)", marker_line_width=1,
        ))
        fig_bar.add_trace(go.Bar(
            x=[f"D{d}" for d in all_doors], y=bar_post_v, name="Posterior",
            marker_color=bar_clrs, marker_line_color="rgba(0,0,0,0.3)", marker_line_width=1,
            text=[f"{v:.1%}" if v > 0.001 else "0%" for v in bar_post_v],
            textposition="outside", textfont=dict(family="JetBrains Mono", size=9, color="#c8c4b9"),
        ))
    ...
    for k in steps:
        ...
        ps_line.append(a); pw_line.append(b)

    fig_line = go.Figure()
    fig_line.add_trace(go.Scatter(x=steps, y=ps_line, mode="lines+markers", name="P(Stay)",
        line=dict(color="#f5a623", width=2.5), marker=dict(size=7, color="#f5a623", line=dict(color="#0d0d14", width=2)),
        hovertemplate="k=%{x}<br>P(stay): %{y:.4f}<extra></extra>"))
    fig_line.add_trace(go.Scatter(x=steps, y=pw_line, mode="lines+markers", name="P(Switch) per door",
        line=dict(color="#4ade80", width=2.5, dash="dash"), marker=dict(size=7, color="#4ade80", line=dict(color="#0d0d14", width=2)),
    ...
    st.markdown(
        ...
        f"the likelihood ratio is <b>(N−1)/(N−1−k) = {n_doors-1}/{n_doors-1-n_opened} = {(n_doors-1)/(n_doors-1-n_opened):.3f}</b>, "
